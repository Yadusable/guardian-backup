...
pub trait ServerService {
    ...
    async fn handle_incoming_request(&mut self, call: impl UnhandledIncomingCall) -> Result<(), Self::Error>;
}
    ...
    async fn handle_incoming_request(&mut self, call: impl UnhandledIncomingCall) -> Result<(), Self::Error> {
        ...
        match call_variant {
            Call::CreateBackup(backup) => {
                self.backup_repository.create_backup(user, backup).await.map_err(|err| ServerServiceError::BackupRepositoryError(err.into()))?;
                call.answer(BackupCreated).await.map_err(|err| ServerServiceError::ResponseError(err.into()))?;
            }
        }
